...
export const Input = ({ className, ...props }: React.ComponentPropsWithoutRef<typeof ShadInput>) => (
    <ShadInput
        className={cn('bg-gray-100 focus:bg-white transition-all placeholder:text-gray-400 h-12', className)}
        {...props}
    />
);
        ...
        <div>
            ...
            <div className="flex">
                <div className="flex bg-gray-100 border rounded-bl-md border-r-0 border-t-0 items-center justify-center px-4">
                    <TypographyP className="text-gray-500 text-xs w-full select-none">{dialCode}</TypographyP>
                </div>
...
export const PinInput = ({ className, ...props }: React.ComponentPropsWithoutRef<typeof ShadInput>) => (
    <ShadInput className={cn('bg-gray-100 focus:bg-white placeholder:text-gray-800 h-12', className)} {...props} />
);

export const MapSearchOverlay = ({ onChange }: { onChange?: (placeDetails: any) => void }) => {
            ...
            <div className="flex flex-col pointer-events-auto" id="address-input">
                <div className="relative w-full flex items-center">
                    <Input
                        ...
                        className={`w-full bg-white drop-shadow-md pr-14 pointer-events-auto ${
                            showSuggestions && 'rounded-b-none z-10'
                        }`}
                        value={text}
                        onChange={evt => {
                        ...
                        {showSuggestions && (
                            <div className=" rounded-lg rounded-t-none bg-white py-2 z-10 border-gray-300 border border-t-0 drop-shadow-md">
                                ...
                                {placePredictions.length === 0 && !isPlacePredictionsLoading && (
                                    <LocationSearch onClick={() => getUserLocation()} isLast>
                                        <div className={'p-2 rounded-full bg-accent'}>
                                            <Image
                                                src="/icons/nav/navigation.svg"
                                                width={16}
                                        ...
                                        <TypographyP>Use my current location</TypographyP>
                                    </LocationSearch>
                                )}
                            </div>
                        )}
...
const LocationSearch = ({
    ...
    return (
        <div
            onClick={onClick}
            className={` z-20 pointer-events-auto transition-colors font-mono flex items-center gap-2 bg-white py-2 px-4 cursor-pointer ${
