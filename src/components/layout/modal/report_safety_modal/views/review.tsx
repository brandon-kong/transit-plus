...
export default function ReportSafetyReviewView() {
    ...
    return (
        <div className="flex flex-col gap-4 mt-4">
            <ScrollArea className={'max-h-[400px] sm:max-h-[500px]'}>
                <div className="flex flex-col gap-4 px-4 py-2 pb-48">
                    <div className="flex flex-col justify-between">
                        <TypographyH4>Review your report</TypographyH4>
                    ...
                    <div className="h-[400px] w-full rounded-xl overflow-hidden">
                        ...
                        >
                            <IndicatorMarker lat={location?.lat || 41.7} lng={location?.lng || -87.5} />
                        </GoogleMapReact>
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                        <div className="flex flex-col gap-2">
                            ...
                            <InputWithLabel disabled label="Date" type="date" value={date?.split('T')[0]} />
                        </div>

                        <div className="flex flex-col gap-2">
                            <TypographyH4>Time</TypographyH4>
                            <InputWithLabel label="Time" disabled type="time" value={time} />
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                        <TypographyH4>Type of incident</TypographyH4>
                        <IncidentCard
                            title={typeOfIncident as string}
                            description={
                                IncidentTypes.find(incident => incident.title === typeOfIncident)?.description || ''
                            }
                        />
                    ...
                    <div className="flex flex-col gap-4">
                        <TypographyH4>Severity of Concern</TypographyH4>

                        <div className="flex w-full">
                            <Button
                                className={`w-full cursor-default rounded-r-none bg-gray-200 hover:bg-gray-200 transition-all text-black ${
                                    severity === 'Low' &&
                                    'ring-2 ring-black z-10 bg-yellow-500 hover:bg-yellow-500 text-white'
                                }`}
                            >
                                Low
                            </Button>
                            <Button
                                className={`w-full cursor-default rounded-none bg-gray-200 hover:bg-gray-200 transition-all text-black ${
                                    severity === 'Medium' &&
                                    'ring-2 ring-black z-10 bg-orange-500 hover:bg-orange-500 text-white'
                                }`}
                            >
                                Medium
                            </Button>
                            <Button
                                className={`w-full cursor-default rounded-l-none bg-gray-200 hover:bg-gray-200 transition-all  text-black ${
                                    severity === 'High' &&
                                    'ring-2 ring-black z-10 text-white bg-red-600 hover:bg-red-600'
                                }`}
                            >
                                High
                            </Button>
                    ...
                    <div className="flex flex-col gap-4">
                        <TypographyH4>Additional Details</TypographyH4>
                        <TextareaWithLabel
                            disabled
                    ...
                    <div className="flex flex-col gap-4">
                        <TypographyH4>Attachments</TypographyH4>
                        ...
                        {files.map((file, index) => (
                            <div key={index} className="flex flex-col gap-2">
                                ...
                                <TypographyP className="text-xs text-gray-500">{file.name}</TypographyP>
                            </div>
                        ))}
                        {files.length === 0 && (
                            <TypographyP className="text-xs text-gray-500">No attachments</TypographyP>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col gap-4">
                        <TypographyP>
                            All reports are anonymous and will be reviewed by our team. If you would like to be
                            contacted, please include your contact information in the additional details.
                        </TypographyP>

                        <TypographyP>
                            By submitting this report, you are agreeing to our{' '}
                        ...
                        {status === 'loading' && <BlackSpinner />}

                        {status === 'unauthenticated' && (
                            <div className="flex flex-col gap-4">
                                <TypographyP className="text-sm">
                                    You must be logged in to submit a report. We use this information to help prevent
                                    abuse of the system.
                                </TypographyP>
            ...
            <DialogFooter>
                <Button onClick={() => setView('severity')} variant={'ghost'}>
                    Back
                </Button>

                <Button onClick={attemptCreateReport} disabled={!confirmed || status !== 'authenticated'}>
                    Submit
                </Button>
            </DialogFooter>
        </div>
...
const IndicatorMarker = ({ lat, lng }: { lat: number; lng: number }) => {
    return (
        <div className="h-10 w-10 translate-x-[-50%] translate-y-[-50%] relative flex items-center justify-center">
...
const IncidentCard = ({ title, description }: { title: string; description: string }) => {
    return (
