    ...
    return (
        <div className="flex flex-col gap-4 mt-4">
            <div className="flex justify-between">
                <TypographyH4>Which train station will you be departing from?</TypographyH4>
            </div>

            <ScrollArea className="max-h-[300px] md:max-h-[500px] h-full">
                <div className="w-full gap-4 p-4 pl-4 gap flex flex-col">
                    <div className="flex flex-col gap-2">
                        <TypographyP className="text-sm font-semibold text-left w-full">Suggested Station</TypographyP>

                        <div
                            className={`${
                                CTATrainLineTypes[closestStation?.line as string] || 'bg-gray-100'
                            } p-4 w-full flex items-center justify-center rounded-lg`}
                        >
                            {closestStation?.name}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {Object.keys(CTATrainLineTypes).map(line => {
                            return (
                                <div
                                    key={line}
                                    className={`${CTATrainLineTypes[line]} p-4 w-full flex items-center justify-center rounded-lg`}
                                >
                                    {line}
                                </div>
                            );
            ...
            <DialogFooter>
                ...
                <Button disabled={!typeOfTransportation} onClick={() => setView('destination')}>
                    Continue
                </Button>
            </DialogFooter>
        </div>
...
const TransportationCard = ({
    ...
    return (
        <Card
            ...
            onClick={onClick}
        >
            <Image src={src} alt={title} width={30} height={30} className="select-none" />

            <TypographyP className={'text-sm font-semibold text-left w-full'}>{title}</TypographyP>
        </Card>
    );
};
