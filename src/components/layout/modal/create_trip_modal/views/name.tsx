...
export default function CreateTripNameView() {
    ...
    return (
        <div className="flex flex-col gap-4 mt-4">
            <ScrollArea className="flex-grow max-h-[300px] md:max-h-[500px]">
                <form onSubmit={attemptCreateTrip} className="flex flex-col gap-4 p-4">
                    <div className="flex justify-between">
                        <TypographyH4>Finally, give your trip a name</TypographyH4>
                    </div>
                    ...
                    {status === 'unauthenticated' && (
                        <div className="flex flex-col gap-4">
                            <TypographyP className="text-sm">
                                You must be logged in to submit a report. We use this information to help prevent abuse
                                of the system.
                            </TypographyP>
            ...
            <DialogFooter>
                <Button variant={'outline'} onClick={() => setView('frequency')}>
                    Back
                </Button>
                <Button disabled={!name || status !== 'authenticated'} onClick={attemptCreateTrip}>
                    Continue
                </Button>
            </DialogFooter>
        </div>
